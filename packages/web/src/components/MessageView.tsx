  ...
  if (message.purpose === "summarization") {
    ...
    return (
      <div className="mx-auto w-full max-w-[720px] px-4 py-2">
        <details className="group rounded-md border border-border bg-bg-sunken px-3 py-2">
  ...
  if (message.role === "user") {
      ...
      return (
        <div className="mx-auto flex w-full max-w-[720px] gap-2.5 px-4 py-1.5">
          ...
          <div className="min-w-0 max-w-[82%]">
            ...
            <div className="space-y-2 rounded-lg border border-border bg-bg-raised px-3 py-2.5 text-[13px] leading-relaxed text-fg">
              {imageBlocks.map((block, i) => block.type === "image" ? <img key={i} src={`data:${block.mimeType};base64,${block.data}`} alt="příloha" className="max-h-64 rounded-md border border-border" /> : null)}
    ...
    return (
      <div className="mx-auto flex w-full max-w-[720px] justify-end px-4 py-1.5">
        <div className="max-w-[78%] rounded-lg bg-fg px-3.5 py-2.5 text-[13px] leading-relaxed text-bg-raised">
          {imageBlocks.map((block, i) => block.type === "image" ? <img key={i} src={`data:${block.mimeType};base64,${block.data}`} alt="příloha" className="mb-2 max-h-64 rounded-md border border-white/10" /> : null)}
