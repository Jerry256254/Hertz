...
import { Avatar, Button, Input, Label, Textarea } from "./ui";
...
export function NewTaskDialog({
  ...
  const createTask = useMutation({
    mutationFn: () =>
      api.post<{ id: string }>(`/projects/${projectId}/tasks`, {
        title,
        description,
        assigneeAgentIds: [...selected],
      }),
    onSuccess: () => {
  ...
  const eligible = agents.filter((a) => a.role !== "manager");
  const canCreate = title.trim().length > 0 && description.trim().length > 0 && selected.size > 0;
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-lg border border-border bg-bg-raised shadow-popover">
          <div className="flex h-12 items-center justify-between border-b border-border px-4">
            <Dialog.Title className="text-sm font-semibold text-fg">New task</Dialog.Title>
          ...
          <div className="p-4">
            ...
            <Textarea
              placeholder="What needs to happen, and any constraints."
              value={description}
            ...
            <div className="mb-4 max-h-56 overflow-y-auto rounded-md border border-border">
              ...
              {eligible.length === 0 && <p className="p-3 text-xs text-fg-subtle">No employees on this project yet.</p>}
            </div>
            ...
            >
              {createTask.isPending ? "Assigning…" : `Assign task${selected.size ? ` (${selected.size})` : ""}`}
            </Button>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
