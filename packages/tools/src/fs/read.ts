...
const inputSchema = z.object({
  ...
  root: z.string().optional().describe("Which root to read from — omit for the shared project root, 'self' for your own personal folder (notes/materials/data), or a folder name from Your folders"),
});
...
export const readFileTool: ToolDef<Input> = {
  name: "read_file",
  description:
    "Read a file, optionally restricted to a line range. Prefer a range for large files — reading the whole file wastes context. Pass root: 'self' to read from your own personal folder instead of the shared project. Text tool — never use desktop_* / browser_* to open an editor; do file work here.",
  inputSchema,
