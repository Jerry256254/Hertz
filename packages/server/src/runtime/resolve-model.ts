 ...
 * stored id is no longer offered, fall back to the provider's default model
 * (or the first scanned one) and persist the correction so the UI shows it.
 * A scan failure never blocks the run — the stored id is used as-is.
 */
