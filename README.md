...
| Forgot password | Run `hzcli passwd <email>` in a terminal (from checkout: `node packages/cli/dist/bin.js passwd <email>`) — works even while the server is stopped; `hzcli users` lists accounts |
| Agent remembers wrong/stale things | `hzcli wipe-memory --agent <name>` erases one agent's memory completely (`--all` for everyone, `--with-skills` to include skills) |
| Want to start over completely | `hzcli factory-reset` deletes everything back to first-install state (asks for typed confirmation) |
